<?php
$query = $this->db->query("SELECT username, kuota, password FROM ukm WHERE username = '".$this->session->userdata("adminBBMK19")."'");
$data = $query->row(); ?>
<ul class="list-group">
	...
	<li class="list-group-item">
		...
		<form action="" method="post" style="margin-bottom: 10px; border-bottom: 1px solid grey">
			<div class="form-group">
				<div class="row">
					<div class="col-md-4">Jumlah Kuota</div>
					<div class="col-md-6">
						<input type="number" disabled="disabled" value="<?= $data->kuota; ?>" class="form-control">
						<font class="text-danger"><i><small>(Hanya dapat diubah setelah konfirmasi ke Neo Telemetri)</small></i></font>
					</div>
					<div class="col-md-2">
						<input type="submit" name="ubahKuota" class="btn btn-warning" value="Ubah" disabled="disabled">
					</div>
				</div>
			</div>
		</form>
