...
class Admin extends CI_Controller
{
	...
	public function index()
	{
		...
		if (!@$this->session->userdata("adminBBMK19")) {
			...
			$this->load->view("ukmpage/login");
		} else {
			$this->alert->msg("success", "Anda sudah login...", "Session anda diaktifkan", 1, base_url("/admin/home"));
		}
	}
	...
	public function home($id = null)
	{
		if (@$this->session->userdata("adminBBMK19")) {
			...
			$this->load->view("ukmpage/header");
			$this->load->view("ukmpage/peserta");
			$this->load->view("ukmpage/footer");
		} else {
	...
	public function notif()
	{
		...
		if (@$this->session->userdata("adminBBMK19")) {
			...
			$this->load->view("ukmpage/header");
			$this->load->view("ukmpage/pemberitahuan");
			$this->load->view("ukmpage/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	public function profile()
	{
		if (isset($_POST["kirim"])) {
			...
			$update = $this->db->query("UPDATE ukm SET deskripsi = '$deskripsi', syarat1 ='$syarat1', syarat2='$syarat2',syarat3='$syarat3', syarat4='$syarat4', syarat5='$syarat5', visi = '$visi', misi = '$misi', motto = '$motto' WHERE username = '" . $this->session->userdata("adminBBMK19") . "'");
			if ($update) {
				$data["update"] = "berhasil";
			} else {
				$data["update"] = "gagal";
		...
		if (@$this->session->userdata("adminBBMK19")) {
			$data["title"] = "Buat Pemberitahuan | BBMK ";
			$this->load->view("ukmpage/header", $data);
			$this->load->view("ukmpage/profile");
			$this->load->view("ukmpage/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin pada halaman ini", "1", base_url());
		}
	}
	...
	public function foto()
	{
		...
		if (@$this->session->userdata("adminBBMK19")) {
			$data["title"] = "Upload Logo| BBMK 2019";
			$this->load->view("head", $data);
			$this->load->view("ukm/template/header");
			$this->load->view("ukm/foto");
			$this->load->view("ukm/template/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	public function peserta($status = null)
	{
		if (@$this->session->userdata("adminBBMK19")) {
			...
			$data["status"] = $status;
			$this->load->view("ukmpage/header");
			$this->load->view("ukmpage/peserta", $data);
			$this->load->view("ukmpage/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	public function verifikasi($status = null)
	{
		if (@$this->session->userdata("adminBBMK19")) {
			$data["title"] = "Daftar Peserta | BBMK 2019";
			if ($status == null) {
			...
			$this->load->view("head", $data);
			$this->load->view("ukmpage/header");
			$this->load->view("ukmpage/verifikasi");
			$this->load->view("ukmpage/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	public function materi($status = null)
	{
		if (@$this->session->userdata("adminBBMK19")) {
			$data["title"] = "Daftar Peserta | BBMK 2019";
			if ($status == null) {
	...
	public function kehadiran($status = null)
	{
		if (@$this->session->userdata("adminBBMK19")) {
			...
			$this->load->view("head", $data);
			$this->load->view("ukmpage/header");
			$this->load->view("ukmpage/kehadiran");
			$this->load->view("ukmpage/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk masuk halaman ini", "1", base_url());
		}
	}
	...
	public function daftarhadir($status = null)
	{
		if (@$this->session->userdata("adminBBMK19")) {
			...
			$this->load->view("head", $data);
			$this->load->view("ukmpage/header");
			$this->load->view("ukmpage/daftarhadir");
			$this->load->view("ukmpage/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	}
	public function statistik($status = null)
	{
		if (@$this->session->userdata("adminBBMK19")) {
			$data["title"] = "Kehadiran Peserta | BBMK 2021";
			if ($status == null) {
			...
			$this->load->view("head", $data);
			$this->load->view("ukmpage/header");
			$this->load->view("ukmpage/statistik");
			$this->load->view("ukmpage/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	public function kelulusanpeserta($status = null)
	{
		if (@$this->session->userdata("adminBBMK19")) {
			...
			$this->load->view("ukmpage/header");
			$this->load->view("ukmpage/kelulusan");
			$this->load->view("ukmpage/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	public function sertifikat($status = null)
	{
		if (@$this->session->userdata("adminBBMK19")) {

			$this->load->view("ukmpage/header");
			$this->load->view("ukmpage/sertifikat");
			$this->load->view("ukmpage/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	public function entry($pid = 1)
	{
		if (@$this->session->userdata("adminBBMK19")) {
			...
			$this->load->view("ukmpage/header");
			$this->load->view("ukmpage/datapeserta", $data);
			$this->load->view("ukmpage/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	public function dokumentasi()
	{
		...
		if (@$this->session->userdata("adminBBMK19")) {
			...
			$this->load->view("head", $data);
			$this->load->view("ukm/template/header");
			$this->load->view("ukm/dokumentasi");
			$this->load->view("ukm/template/footer");
		} else {
			$this->alert->msg("error", "Oops...", "Anda tidak memiliki izin untuk menghapus halaman ini", "1", base_url());
		}
	...
	{
		if (@$this->session->userdata("adminBBMK19")) {
			$this->load->view('report/ukm/laporan_verif');
		} else {
			$this->alert->msg("error", "Oops..", "Anda tidak memiliki izin untuk mengakses halaman ini", 1, base_url() . "/superadmin/list");
		}
	}

	public function downloadkelulusan()
	{
		if (@$this->session->userdata("adminBBMK19")) {
			$this->load->view('report/ukm/laporan_kelulusan');
		} else {
			$this->alert->msg("error", "Oops..", "Anda tidak memiliki izin untuk mengakses halaman ini", 1, base_url() . "/superadmin/list");
		}
	}
	...
	{
		if (@$this->session->userdata("adminBBMK19")) {
			$this->load->view('report/ukm/laporan_kehadiran');
		} else {
			$this->alert->msg("error", "Oops..", "Anda tidak memiliki izin untuk mengakses halaman ini", 1, base_url() . "/superadmin/list");
		}
	}
